...
/*
    ...
    1、创建（管理员）
    2、插入（传入数据）
    3、查询（根据学生id、课程id查询课程发布的作业成绩详情列表）
    4、查询（根据学生id、课程id和作业次数查询发布的作业成绩详情）
*/
contract HomeworkGrade{
    ...
    constructor() public{
        ...
        _field[2] = "classid";
        _field[3] = "classname";
        _field[4] = "homeworknumber";
        _field[5] = "marktime";
        _field[6] = "grade";
        TableFactory tf = TableFactory(0x1001);
        tf.createTable(_tablename,"key",
        "studentid,content,classid,classname,homeworknumber,marktime,grade");
    }
    ...
    function checkRecordExist(string studentid,string classid,string homeworknumber) public view returns(int){
        TableFactory tf= TableFactory(0x1001);
    ...
    function insert(string[] info) public returns(int){
        ...
        // //检查是否已经有记录
        if(checkRecordExist(info[0],info[2],info[4])>0) return 0;
        //插入数据
        TableFactory tf = TableFactory(0x1001);
        Table table = tf.openTable(_tablename);
    ...
    //4、查询（根据课程id和作业次数查询发布的作业成绩详情）
    function selectByStudentidAndClassidAndNumber(string studentid,string classid,string homeworknumber) public view returns(string[] memory){
        TableFactory tf= TableFactory(0x1001);
        Table table = tf.openTable(_tablename);
